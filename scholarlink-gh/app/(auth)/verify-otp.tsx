...
export default function VerifyOtpScreen() {
  ...
  const handleKey = (key: string, index: number) => {
    if (key === 'Backspace' && !codes[index] && index > 0) refs.current[index - 1]?.focus();
  };
  ...
  return (
    <Screen>
      <SectionHeader title="Verify Email" subtitle={`Enter the 6-digit code sent to ${email}`} />
      ...
      <AppButton title={resending ? 'Sending…' : 'Resend Code'} onPress={handleResend} variant="ghost" loading={resending} />
    </Screen>
  );
}

const styles = StyleSheet.create({
  codeRow: { flexDirection: 'row', gap: 10, justifyContent: 'center', marginVertical: 16 },
  codeBox: {
  ...
  codeBoxFilled: { borderColor: colors.primary, backgroundColor: colors.surfaceMuted },
});
